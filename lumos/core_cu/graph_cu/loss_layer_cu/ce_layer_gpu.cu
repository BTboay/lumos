...
{
    cross_entropy_kernel<<<(len+BLOCK-1)/BLOCK, BLOCK>>>(input, truth, len, space);
}

void delta_cross_entropy_gpu(float *input, float *truth, int len, float *space)
{
    delta_cross_entropy_kernel<<<(len+BLOCK-1)/BLOCK, BLOCK>>>(input, truth, len, space);
}